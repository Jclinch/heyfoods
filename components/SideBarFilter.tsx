...
const SideBarFilter: React.FC<SideBarFilterProps> = ({
  ...
  return (
    <div className="w-[120px] md:w-[256px] p-4 md:ml-[35px] mt-[60px] text-[8px] md:text-[24px] ">
      ...
      <p className="text-gray-500 mb-4 text-[10px] md:text-[24px]">({sampleData.length} Stores)</p>
        ...
        <ul className="space-y-2">
          {sortOptions.map((option) => (
            <li key={option.value} className="flex items-center gap-2 mb-4">
              ...
              <label htmlFor={option.value} className="text-[8px] md:text-[18px] text-base font-[500] cursor-pointer">
                {option.label}
              </label>
            </li>
          ))}
